...
<p><a href=/{$current_php_script}.php?edit=1&id={$temp_array.$key5.id}>{$temp_array.$key5.title}</a> | 
        {$temp_array.$key5.price} | 
        {$temp_array.$key5.user_name} | 
        <a href=/{$current_php_script}.php?del=1&id={$temp_array.$key5.id}>Удалить</a></p>
{/foreach}
{/if}   
